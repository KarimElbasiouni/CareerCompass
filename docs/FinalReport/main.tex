...
We selected BERT checkpoints using validation macro-F1. The validation split appears to contain a higher proportion of minority classes or harder disambiguation cases than the test set. As a result, early stopping likely triggered before BERT fully converged on patterns that generalize beyond these difficult examples.

The validation set has 350 samples spread across 36 classes, or about 9.7 examples per class on average. With such a small per-class sample size, random variation in which resumes land in validation versus test can significantly affect scores. If validation happens to include more ambiguous resumes (e.g., borderline \emph{Python Developer} vs.\ \emph{Data Science}), the lower validation score may reflect a harder split rather than a weaker model.

BERT was fine-tuned under strict computational constraints: 2 epochs, batch size 4, and a maximum sequence length of 128 tokens on CPU. These settings limited convergence and underutilized the model's capacity. In contrast, the SVM's regularization parameter \(C\) was tuned directly on validation via grid search. This asymmetry makes it easier for SVM to look strong on validation.
...
Many technical roles use nearly identical vocabularies. \emph{Python Developer} and \emph{Data Science} resumes both mention pandas, NumPy, scikit-learn, and Jupyter. The real difference is often in how those tools are used: building production APIs versus performing analytics and modeling. Both models struggle on resumes that mix these signals.
...
Many resumes describe a career path instead of a single static role. For example, a candidate might have 5 years as a QA Engineer followed by 1 year in DevOps. Our ground truth labels such resumes as \emph{DevOps Engineer} (based on the most recent role), while both models often predict \emph{Testing} or a related QA title, reflecting the majority of the experience. This exposes a limitation of single-label classification for time-evolving careers.

Some resumes use vague or high-level language with few concrete skills or tools. In these cases, there is not enough signal to distinguish between similar titles, and both models tend to default toward more frequent or generic classes.

\subsection{Future Directions}

Based on these observations, several directions appear promising for improving classification performance.

Instead of predicting fine-grained job titles in one step, we could first predict a coarse job family (e.g., Engineering, Healthcare, Sales), then refine the prediction within that family. This would reduce confusion between distant categories and focus the model on finer distinctions only when appropriate.

Allowing resumes to have multiple labels would better reflect real-world career paths and hybrid roles, such as a combination of \emph{Full-Stack Developer} and \emph{DevOps Engineer}. This would address many of the errors we observe on resumes that span several roles over time.
