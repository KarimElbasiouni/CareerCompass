...
\end{equation}
which penalises misclassified points outside the margin while regularising model weights. In practice, the scikit-learn implementation expresses \(\lambda\) as \(1 / C\); we tuned \(C \in \{0.25, 0.5, 1.0, 2.0\}\) and found \(C = 2.0\) delivered the best validation results. Optimisation relies on coordinate-descent (LibLinear), a deterministic convex solver that efficiently updates each parameter to minimise the global loss. Class weighting compensates for mild imbalance among the thirty-six labels.
...
The primary limitation of this setup is its reliance on lexical features: semantically similar phrases may be treated as unrelated, and the model produces uncalibrated decision margins rather than probabilities. These constraints motivate the next step, introducing a BERT-based contextual encoder fine-tuned with a softmax output and cross-entropy loss, optimised via AdamW. This comparison will reveal whether richer semantic representations improve generalisation beyond the classical linear baseline.

\section{Results and Evaluation}

The evaluation followed a standardised 80 / 10 / 10 stratified train, validation, and test split across 3{,}500 resumes representing 36 canonical job titles. Splits were fixed by random seed to ensure reproducibility and balanced class distribution. All TF-IDF features (1--2 grams, min\_df = 3, max\_df = 0.9) were fit on the training subset only, preventing data leakage, and corresponding label encodings were regenerated consistently across runs.
...
The majority-class baseline, which always predicts the most common training title, reached only 0.057 accuracy and 0.003 macro-F1 on both validation and test splits, establishing a lower bound for comparison. By contrast, the Linear SVC trained on TF-IDF features achieved 0.900 validation accuracy (macro-F1 0.936) and 0.886 test accuracy (macro-F1 0.924) with \(C = 2.0\), while Top-3 accuracy exceeded 0.97 on both splits. The close alignment of validation and test results indicates good generalisation and minimal overfitting. The corresponding confusion matrices in Figures~\ref{fig:confusion-val} and \ref{fig:confusion-test} confirm this trend: strong diagonal dominance indicates high per-class accuracy, while small off-diagonal clusters highlight overlap between related titles such as \emph{Python Developer} and \emph{Data Science}.

\begin{center}
\refstepcounter{figure}
\includegraphics[width=\columnwidth]{../../runs/svm_tfidf/confusion_val.png}\\
{\small \textbf{Figure \thefigure:} Confusion matrix on the validation set showing class-wise prediction counts and normalised row percentages for the Linear SVC baseline.}
...
Our TA's primary feedback emphasised the need for clearer and more varied visualisations of results, suggesting the addition of summary graphs or alternative plots beyond the confusion matrix to make class-level performance easier to interpret. This aligns with our broader focus on interpretability and transparent model evaluation, motivating the inclusion of calibration and ranking-based visual metrics in future deliverables.

Reflecting on this, the team identified that while the current evaluation pipeline reports numerical accuracy and F1 scores, these metrics alone obscure which classes contribute most to performance variance. In the next phase, we plan to incorporate result graphs such as per-class F1 bars, calibration curves, and Top-\(k\) recall plots to provide a clearer picture of prediction reliability. We also intend to extend the baseline by implementing a BERT-based contextual model, which will be evaluated on the same fixed splits and metrics to isolate the contribution of semantic embeddings.

Moving forward, our focus will be on enhancing interpretability, experimenting with calibrated probability outputs, and validating whether transformer-derived embeddings improve performance consistency across minority classes. The feedback confirmed that our implementation direction is sound but encouraged deeper visualisation and comparative analysis to strengthen communication of model behaviour.

% \section*{Limitations}

\section*{Team Contributions}

Imran Chowdhury implemented the data cleaning, TF-IDF feature extraction, and Linear SVC training pipeline, producing the final evaluation metrics and confusion matrices. Karim Elbasiouni refined the baseline models, updated visualisation outputs, and coordinated feedback with the teaching assistant. Rami Abu Sultan compiled the progress report, integrated code results, and finalised the submission in \LaTeX{}.
